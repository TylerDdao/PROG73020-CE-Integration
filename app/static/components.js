...
loadComponent('modal-placeholder', '/static/sub-box.html');
...
document.addEventListener('keydown', function(e) {
  ...
  let currentProduct = { name: '' , current_id: 0};
  let qty = 1;
  ...
  function openSubModal(name, product_id) {
    console.log("1. Function triggered. Name:", name, "ID:", product_id);
    currentProduct = { name, product_id };
    qty = 1;
 
    const modal = document.getElementById('subModal');
    const nameLabel = document.getElementById('modalProductName');
    
    console.log("2. Element check - Modal found:", !!modal, "| Label found:", !!nameLabel);

    if (!modal) {
        console.error("CRITICAL: Element with ID 'subModal' was not found in the DOM.");
        return;
    }
    ...
    console.log("3. 'open' class added. Current classes:", modal.className);

    const computedStyle = window.getComputedStyle(modal);
    console.log("4. Final CSS Display:", computedStyle.display, "| Z-Index:", computedStyle.zIndex);
